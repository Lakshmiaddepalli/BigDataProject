...
var df4 = sqlContext.read.format("csv").option("header", "true").load("hdfs:///user/sla410/crimedatabigdataproject/311_Service_Requests_-_Vacant_and_Abandoned_Buildings_Reported_-_Historical.csv")
df4.printSchema()
...
df4.filter(df4("LOCATION OF BUILDING ON THE LOT").isNull || df4("LOCATION OF BUILDING ON THE LOT") === "" || df4("LOCATION OF BUILDING ON THE LOT").isNaN).count() //8777 //will drop dont require
df4.filter(df4("IS THE BUILDING DANGEROUS OR HAZARDOUS?").isNull || df4("IS THE BUILDING DANGEROUS OR HAZARDOUS?") === "" || df4("IS THE BUILDING DANGEROUS OR HAZARDOUS?").isNaN).count() //65113 //will drop dont require
df4.filter(df4("IS BUILDING OPEN OR BOARDED?").isNull || df4("IS BUILDING OPEN OR BOARDED?") === "" || df4("IS BUILDING OPEN OR BOARDED?").isNaN).count() //8259 //will drop dont require
df4.filter(df4("IF THE BUILDING IS OPEN, WHERE IS THE ENTRY POINT?").isNull || df4("IF THE BUILDING IS OPEN, WHERE IS THE ENTRY POINT?") === "" || df4("IF THE BUILDING IS OPEN, WHERE IS THE ENTRY POINT?").isNaN).count() //30678 //will drop dont require
df4.filter(df4("IS THE BUILDING VACANT DUE TO FIRE?").isNull || df4("IS THE BUILDING VACANT DUE TO FIRE?") === "" || df4("IS THE BUILDING VACANT DUE TO FIRE?").isNaN).count() //9123 //replace empty values with none
df4.filter(df4("ANY PEOPLE USING PROPERTY? (HOMELESS, CHILDEN, GANGS)").isNull || df4("ANY PEOPLE USING PROPERTY? (HOMELESS, CHILDEN, GANGS)") === "" || df4("ANY PEOPLE USING PROPERTY? (HOMELESS, CHILDEN, GANGS)").isNaN).count() //8551 //replace empty values with none
...
df4.filter(df4("X COORDINATE").isNull || df4("X COORDINATE") === "" || df4("X COORDINATE").isNaN).count() //41 //drop
df4.filter(df4("Y COORDINATE").isNull || df4("Y COORDINATE") === "" || df4("Y COORDINATE").isNaN).count() //41 //drop
df4.filter(df4("Ward").isNull || df4("Ward") === "" || df4("Ward").isNaN).count() //45 //drop
...
df4 = df4.withColumn("DATE SERVICE REQUEST WAS RECEIVED", todate($"DATE SERVICE REQUEST WAS RECEIVED"))
